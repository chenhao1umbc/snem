...
# %% test nem
    ...
    def nem_func(x, J=3, Hscale=1, Rbscale=100, max_iter=50, lamb=0, seed=0, model='', show_plot=False):
        if model == '':
    ...
    res_mse, res_corr = [], []
    for id in range(1,6):
        location = f'../../Hpython/data/nem_ss/models/model_4to50_em20_151epoch_1H_100Rb_v{id}.pt'
        for i in range(I):
            x = torch.from_numpy(x_all[i]).permute(1,2,0)
            MSE, CORR = [], []
            for ii in range(20):  # for diff initializations
            ...
            res_corr.append(CORR)
            print(f'finished {i} samples')
        torch.save((res_mse, res_corr), f'nem_20iter_v{id}.pt')

    res_mse, res_corr = [], []
    for id in range(1,6):
        location = f'../../Hpython/data/nem_ss/models/model_4to50_31epoch_1H_100Rb_v{id}.pt'
        for i in range(I):
            x = torch.from_numpy(x_all[i]).permute(1,2,0)
            MSE, CORR = [], []
            for ii in range(20):  # for diff initializations

...
aoa = (torch.rand(1,J)-0.5)*90 # in degrees
power_db = torch.rand(1, 3)*max_db # power diff for each source

def get_steer_vec(aoa, n_channel, J):
    """get 'real number' steer vec

    Args:
        aoa ([type]): [description]
        n_channel ([type]): [description]
        J ([type]): [description]
    """
